    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if not IsPedInAnyVehicle(PlayerPedId(), 0) then
                if speed > 100 then
                    TriggerServerEvent("UltimateAC:BanPlayer", string.format(Locale[Config.Locale]["BanMessage"],
...
if Config.AntiWeaponSpawning then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if armor > Config.MaxArmor then
                TriggerServerEvent("UltimateAC:BanPlayer",
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if (playercoords.x > 0 or playercoords.x < 0) then
                ...
                if GetEntityHeightAboveGround(playerPed) > 25 and not IsPedInAnyVehicle(playerPed, false) and not IsPedInParachuteFreeFall(playerPed) and not IsPedFalling(playerPed) and not IsPedJumpingOutOfVehicle(playerPed) and not IsEntityVisible(playerPed) and not IsPlayerDead(playerPed) and GetPedParachuteState(playerPed) == -1 then
...
if Config.AIFolderModified then
    Citizen.CreateThread(function()
        while true do
            for i = 1, #ListWeaponsHashes do
                ...
                if dmg_mod > 1.1 or accuracy_mod > 1.2 then
                    TriggerServerEvent("UltimateAC:BanPlayer", string.format(Locale[Config.Locale]["BanMessage"],
                        string.format(Locale[Config.Locale]["AIFolderModified"], ListWeaponsHashes[i], dmg_mod,
                            accuracy_mod)), "AntiCheat", nil, string.format(Locale[Config.Locale]["AIFolderModified"],
                        ListWeaponsHashes[i], dmg_mod, accuracy_mod))
                end
            end
            Citizen.Wait(10000)
        end
...
if Config.AntiBlacklistVehicles then
    Citizen.CreateThread(function()
        while true do
            local ped = PlayerPedId()
            ...
            for _, v in pairs(Blacklist["Vehicles"]) do
                if vehicleModel == GetHashKey(v) then
                    TriggerServerEvent("UltimateAC:BanPlayer", string.format(Locale[Config.Locale]["BanMessage"],
                        string.format(Locale[Config.Locale]["BlacklistCar"], vehicleName)), "AntiCheat", nil,
                        string.format(Locale[Config.Locale]["BlacklistCar"], vehicleName))
                end
            end
            Citizen.Wait(5000)
        end
...
for _, eventName in pairs(Blacklist["Events"]) do
    ...
    AddEventHandler(eventName, function()
        CancelEvent()
        TriggerServerEvent("UltimateAC:BanPlayer", string.format(Locale[Config.Locale]["BanMessage"], string.format(
